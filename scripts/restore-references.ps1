...
# Archived native/DNS/proxy/marketplace: -IncludeArchived
...
foreach ($entry in $entries) {
    ...
    $isEnabled = $true
    if ($null -ne $entry.PSObject.Properties['enabled']) {
        $isEnabled = [bool]$entry.enabled
    }
    if (-not $isEnabled -and -not $IncludeArchived) {
        Write-Host "SKIP archived $($entry.id) (use -IncludeArchived)"
        continue
    }
