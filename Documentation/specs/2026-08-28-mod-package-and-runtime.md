...
- `runtime: native` -- `prism.*` plus CSS plus optional browser filters.
- `runtime: userscript` -- legacy; out of v1 tracers (import path may be stubbed).

Compiled JS in the zip is what runs. TypeScript is never `eval`'d. Pack lints (heuristics for `fetch`, `eval`, `document`) are **warnings**. Enforcement is the capability gate at runtime.
...
Missing grant: method absent or throw. Obfuscation does not bypass the gate.
