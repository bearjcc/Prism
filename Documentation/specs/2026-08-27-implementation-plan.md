# Implementation plan (historical; host and gateway)

## Status

**Superseded for v1.** Do not execute this file as the current work queue.

v1 sequencing: `Documentation/specs/2026-08-28-extension-v1-plan.md`
v1 package/runtime: `Documentation/specs/2026-08-28-mod-package-and-runtime.md`
Decision: `Documentation/adr/0002-extension-first-v1.md`

This file remains the backlog for native host, installer, gateway, registry, website, Firefox product, and related ADRs. Reopen a phase only after an ADR says the extension tracers are done and the host has a job.

---

Originally proposed. Derived from `Documentation/architecture.md` and `Documentation/specs/2026-08-27-repo-layout.md`.

This plan sequences work only. It does not add product scope and does not re-decide anything under "Locked decisions" in the architecture except where ADR 0002 defers desktop-as-authority. Where the architecture lists an open question, this plan schedules an ADR instead of assuming an answer.
...
- [x] ADR: Prism's own licence, decided before any copyleft reference code is considered for reuse. -- `Documentation/adr/0001-project-licence.md` (Accepted 2026-08-27).
- [ ] ADR: host implementation language for `apps/native/host` (portable engine, must build for Windows, Linux, macOS and Android without Electron or a JVM). Deferred: ADR 0002 (native host is not a v1 work queue).
- [ ] ADR: native UI toolkit for `apps/native/ui`, including how tray mode and window mode share one binary. Deferred: ADR 0002.
- [x] ADR: extension build toolchain for `apps/extension`, including how `targets/chrome` and `targets/firefox` stay thin shims over one `src`. -- `Documentation/adr/0003-extension-build-toolchain.md` (Accepted 2026-08-29).
- [ ] ADR: local state store used by the host (single writer, immutable revisions, crash safe). Deferred: ADR 0002.
- [x] ADR: mod expression language for declarative transforms, and whether bounded WASM is in or out of the first runtime. -- `Documentation/adr/0004-mod-expression-language.md` (Accepted 2026-08-29). No CEL/JSON-logic/jq in the first runtime; bounded WASM out.
- [ ] ADR: signature scheme and key hierarchy for publisher, registry and host update signing. Deferred: ADR 0002 (registry and host signing are not a v1 work queue).
- [ ] ADR: whether the first gateway integrates an existing DNS product or ships a native resolver. Deferred: ADR 0002.

### Repository and toolchain

- [x] Choose and commit one task runner at the repo root that builds, tests and lints every app and package with one command per verb. -- root `package.json` npm workspaces: `npm run build`, `npm test`, `npm run lint`.
- [x] Commit formatter and linter configuration for each language chosen, enforced in CI rather than by convention. -- `eslint.config.js`; `npm run lint` in `.github/workflows/ci.yml`.
- [x] Commit an editor and line-ending configuration that keeps tracked files ASCII with stable newlines on Windows checkouts. -- `.editorconfig` (UTF-8, LF, final newline).
- [x] Add a licence file and per-package licence headers consistent with the licence ADR. -- root `LICENSE` (AGPL-3.0-only); workspace `package.json` `"license"` fields.
- [x] Add `CONTRIBUTING.md` covering the checkbox protocol in this file, commit conventions and the References rule. -- repo-root `CONTRIBUTING.md`.

### Continuous integration

- [x] CI job: build every app and package on Windows and Linux. -- `ci.yml` `build` matrix `ubuntu-latest` and `windows-latest` runs `npm run build`.
- [x] CI job: unit tests with coverage reported but not gated in Phase 0. -- `npm test` (vitest) on the same matrix. Coverage is not collected; the job is not coverage-gated.
- [x] CI job: lint and format check. -- `npm run lint` (`eslint` plus `scripts/check-text.mjs`).
- [x] CI job: verify `references.lock.json` restores and SHAs match, failing closed. -- later plan Phase I: `scripts/verify-references-lock.mjs` in `npm run lint`.
- [x] CI job: reject non-ASCII bytes and US spellings from an agreed word list in tracked text. -- `scripts/check-text.mjs` and `scripts/us-spellings.txt`.
- [x] CI job placeholder that will run the engine against `mods/*` once Phase 1 lands, failing loudly while empty rather than passing silently. -- later plan Phase Q: `scripts/check-mods-engine.mjs` (CLI after `tsc`) and `scripts/check-mods-engine.test.mjs` (`npm test` fails closed if `mods/*/prism.yaml` is missing).

### Documentation

- [x] Add `Documentation/adr/README.md` describing the ADR format and index.
- [x] Add a traceability table mapping each architecture requirement to the phase that satisfies it, so gaps are visible. -- `Documentation/traceability.md`.
...
- CI fails when the References lockfile is tampered with. -- later plan Phase I (`scripts/verify-references-lock.mjs`).
...
- [ ] Define declarative transforms and event rules using the chosen expression language. -- ADR 0004: first runtime has no separate expression language; transforms are sanitised CSS, DNR filter lists, extension-owned extractors, and `prism.*` orchestration (see `Documentation/specs/2026-08-28-mod-package-and-runtime.md`). Not a reason to mark the rest of Phase 1 done.
...
- [ ] Implement capability versioning and a capability-diff function producing human-readable increase or decrease results. Schema-only `capabilityDiff` is in `packages/schema` (later plan Phase S). Capability versioning, hosted registry, and signing remain later.
...
- [x] Implement a compiled package cache in extension storage with validation on read, not only on write. -- later-plan Phase X (2026-08-29); remainder of this Phase 2 section stays open.
...
- [x] Implement allow paste and standard input events. -- later Phase K.
- [x] Implement suppression of unsolicited popup creation. -- later Phase M.
- [x] Implement stable title, freezing or constraining title mutation. -- later Phase M.
- [x] Implement autoplay constraint. -- later Phase P (`behaviour.autoplay`).
- [x] Implement scroll-lock release. -- later Phase M.
- [x] Implement modal and chatbot-popup suppression. -- later Phase N.
- [x] Implement consent-interface rejection. -- later Phase N.
- [x] Implement the predefined same-origin user action primitive with an explicit allowlist of actions. -- later Phase N.
- [x] Define hard runtime safety invariants that no site exception can override, and test each one. -- later Phase N/P; origin exception cannot grant eval, page fetch, or extractor HTML.
- [x] Implement a global policy panel in the extension covering these policies with per-origin overrides. -- later Phases K-P popup panel.
- [x] Add a browser-compatibility matrix test for each policy, recording where enforcement is partial. -- `apps/extension/README.md` plus Phase N/P tests; not a live multi-browser CI matrix.
...
- [ ] Implement the current-page panel listing every visual, behavioural and network rule affecting the origin. -- later Phase W covers the extension popup (`apps/extension/src/page-activity.ts`); host-shared schema still later.
...
- [ ] Implement allow once for the current session. -- later Phase Y covers the extension (`apps/extension/src/session-exception.ts`); host-shared duration still later.
...
- [x] Implement UserCSS parsing and a separate sanitisation pass. -- `packages/schema` `compileUserCss`.
- [x] Disclose or reject external URLs, imports, generated content, overlays and automatic update sources by policy. -- later Phase L fail-closed.
- [x] Map sanitised UserCSS onto visual capabilities rather than raw style injection where possible. -- later Phase V `mapUserCss` (`packages/schema/src/usercss-map.ts`).
- [x] Add a corpus test over real UserCSS styles recording accepted, sanitised and rejected counts. -- later Phase L.

### Userscripts

- [x] Implement restricted legacy execution in an isolated user-script world where the browser supports one. -- later Phase O `world: "USER_SCRIPT"`.
- [x] Constrain grants, destinations, remote dependencies and execution world, and show the constraints in the UI. -- later Phase O.
- [ ] Implement unrestricted legacy mode, disabled by default, behind an explicit warning stating it has Tampermonkey-like authority. -- not shipped (later plan: security review).
- [x] Label trust level visibly wherever a legacy item appears, never alongside native safety claims. -- later Phase O popup.
