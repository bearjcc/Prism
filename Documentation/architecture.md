...
- Apply DNS, firewall and private-service policies outside the browser where appropriate.
...
The browser extension contains the reviewed execution primitives. Native Prism packages contain declarative data interpreted by those primitives. The registry cannot introduce a new execution path without an extension release.
...
The local service is the preferred authority on devices with the desktop application installed.
...
The desktop application is the management and authoring UI over the local service.
...
- Support immediate hot reload from the local service.
- Run restricted legacy userscripts in clearly labelled compatibility modes.

The extension communicates with the desktop service through Chrome Native Messaging. Page content scripts communicate with the extension service worker, which mediates local and network capabilities.

The extension does not rely on HTML injection by a local proxy.

### Prism gateway

The optional gateway enforces network and private-service policy.
...
Native mods are declarative packages interpreted by reviewed Prism runtime primitives.
...
- Declarative transforms and event rules.
- Bundled resources.
...
Native mods cannot call arbitrary `fetch`, `eval`, page JavaScript or operating-system APIs.
...
- The desktop service is the preferred local state authority.
- The extension uses Native Messaging rather than proxied page traffic for desktop communication.
...
- Native mods are declarative data interpreted by reviewed runtime primitives.
...
- Exact native mod schema and expression language.
- Which semantic primitives belong in the first runtime.
- How semantic site adapters are maintained and versioned.
- Whether native packages can contain bounded WASM and under what isolation.
