...
- **Kitten egress:** `network.egress` on `prism.kitten-ad-replace` is off by default. Enabling it still goes through `prism.net.request` (the extension broker), not page `fetch`. The broker GETs `https://cataas.com/cat` and returns a sanitised image URL. Bundled kitten images remain the fallback.
